  ...
  return (
    <AppShell>
      <section className="mb-16">
        ...
        <div
          className="mt-2 flex flex-wrap items-start gap-y-6"
          style={{ columnGap: 56 }}
        >
          {groupedStacks.map((group) => (
            ...
            >
              <div className="mb-1.5 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
                <span className="flex h-6 w-6 items-center justify-center rounded-lg border border-border/80 bg-secondary/40">
                  {group.icon}
                </span>
                {group.label}
              ...
              <ActionTodoStack
                ...
                onExpandedChange={(open) => setExpandedGroup(open ? group.id : null)}
              />
            </div>
          ))}
        </div>
